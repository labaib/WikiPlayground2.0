import { randomItem, getWikiToken, editWikiItem } from "./js/wiki.js";
...
const main = async () => {
    ...
    itemId = wikiResponse.results.bindings[0].item.value.split("/").pop()
    ...
    if (candidates.length === 0) {
        listGroupElement.className = "list-group list-group-horizontal overflow-x-auto justify-content-center bg-transparent"
    ...
    } else if (candidates.length > 0 && candidates.length < 4) {
        listGroupElement.className = "list-group list-group-horizontal overflow-x-auto justify-content-center bg-transparent"
    }
    ...
    candidates.forEach(entity => {
        ...
        <div class="card text-center shadow mb-2" style="overflow: hidden; width: fit-content;">
            <div class="card-img-top p-0">
                <iframe src="${opacFrameUrl}" height="600" width="425" style="margin-top: -260px;" title="Opac candidate iFrame"></iframe>
            </div>
            <div class="card-body">
...
editBtn.addEventListener('click', async (event) => {
    ...
    // verfifcare token e nel caso generare alert per eseguire login in wikidata
